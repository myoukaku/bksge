 ...
 *	@brief	2進接頭辞
 */
using kibi = bksge::ratio<(1LL << 10)>;
using mebi = bksge::ratio<(1LL << 20)>;
using gibi = bksge::ratio<(1LL << 30)>;
using tebi = bksge::ratio<(1LL << 40)>;
using pebi = bksge::ratio<(1LL << 50)>;
using exbi = bksge::ratio<(1LL << 60)>;
#if INTMAX_MAX / (1LL << 30) >= (1LL << 40)	// INTMAX_MAX >= (1LL << 70)
using zebi = bksge::ratio<(1LL << 70)>;
#define BKSGE_UNITS_HAS_ZEBI
#endif
#if INTMAX_MAX / (1LL << 30) >= (1LL << 50)	// INTMAX_MAX >= (1LL << 80)
using yobi = bksge::ratio<(1LL << 80)>;
