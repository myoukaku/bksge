/**
 *	@file	unit_test_fnd_span_as_bytes.cpp
 *
 *	@brief	bksge::as_bytes() のテスト
 *
 *	@author	myoukaku
 */
...
void test(Span sp)
{
	BKSGE_ASSERT_NOEXCEPT(bksge::as_bytes(sp));

	auto sp_bytes = bksge::as_bytes(sp);
	using SB = decltype(sp_bytes);
	ASSERT_SAME_TYPE(bksge::byte const, typename SB::element_type);
