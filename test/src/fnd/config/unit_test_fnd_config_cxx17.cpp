/**
 *	@file	unit_test_fnd_config_cxx17.cpp
 *
 *	@brief	
 *
 *	@author	myoukaku
...
BKSGE_WARNING_PUSH()
#if defined(BKSGE_CLANG_VERSION) && (BKSGE_CLANG_VERSION >= 30600) && !defined(__APPLE__)
BKSGE_WARNING_DISABLE_CLANG("-Wfuture-compat")
#endif
...
BKSGE_WARNING_PUSH()
#if defined(BKSGE_CLANG_VERSION) && (BKSGE_CLANG_VERSION >= 30700) && !defined(__APPLE__)
BKSGE_WARNING_DISABLE_CLANG("-Wbraced-scalar-init")
#endif
...
#else
BKSGE_WARNING_PUSH()
BKSGE_WARNING_DISABLE_MSVC(4290)	// C++ の例外の指定は無視されます。
BKSGE_WARNING_DISABLE_GCC("-Wdeprecated")
