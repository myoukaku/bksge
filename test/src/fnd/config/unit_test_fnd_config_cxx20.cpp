...
#if defined(BKSGE_HAS_CXX20_INIT_CAPTURES)
...
GTEST_TEST(ConfigTest, Cxx20DeprecateImplicitCaptureCopyThisTest)
{
	struct X
	{
		...
		{
			int value = 3;
#if defined(BKSGE_HAS_CXX20_DEPRECATE_IMPLICIT_CAPTURE_COPY_THIS)
			auto x = [=, this] {
				return value + g();
			};
#else
...
#if defined(BKSGE_HAS_CXX20_USING_ENUM)
...
#if defined(BKSGE_HAS_CXX20_IS_CONSTANT_EVALUATED)
...
#if !defined(BKSGE_HAS_CXX20_DEPRECATE_COMMA_IN_SUBSCRIPTING_EXPRESSIONS)

BKSGE_WARNING_PUSH();
BKSGE_WARNING_DISABLE_MSVC(4709);	// comma operator within array index expression
BKSGE_WARNING_DISABLE_CLANG("-Wdeprecated-comma-subscript");
...
// TODO
#if 0//defined(BKSGE_HAS_CXX20_ACCESS_CHECKING_ON_SPECIALIZATIONS)
...
{
#if defined(BKSGE_HAS_CXX20_ARRAY_SIZE_DEDUCTION_NEW)
	int* p = new int[]{1,2,3};
#else
...
#if defined(BKSGE_HAS_CXX20_DESTROYING_DELETE)
...
#if defined(BKSGE_HAS_CXX20_NESTED_INLINE_NAMESPACES)
...
#if defined(BKSGE_HAS_CXX20_COROUTINES)
...
#if defined(BKSGE_HAS_CXX20_LAMBDAS_IN_UNEVALUATED_CONTEXTS)
...
#if defined(BKSGE_HAS_CXX20_CONDITIONALLY_TRIVIAL_SPECIAL_MEMBER_FUNCTIONS)
...
#if defined(BKSGE_HAS_CXX20_ADL_AND_FUNCTION_TEMPLATES)
...
#if defined(BKSGE_HAS_CXX20_CONST_MISMATCH_WITH_DEFAULTED_COPY_CONSTRUCTOR)
...
#if defined(BKSGE_HAS_CXX20_CONSTEXPR_IN_DECLTYPE)
