...
GTEST_TEST(TupleTest, TupleLastTest)
{
	...
	{
		BKSGE_CXX14_CONSTEXPR auto t1 = bksge::tuple_last(std::make_tuple(1, 2, 3, 4, 5.5, 6.5f, 7));
	...
	{
		BKSGE_CXX14_CONSTEXPR auto t1 = bksge::tuple_last(std::make_tuple(Noncopyable(1), Noncopyable(2)));
	...
	{
		BKSGE_CXX14_CONSTEXPR auto t1 = bksge::tuple_last(std::make_tuple(42));
