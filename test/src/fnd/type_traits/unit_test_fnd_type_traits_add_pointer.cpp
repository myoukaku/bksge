...
GTEST_TEST(TypeTraitsTest, AddPointerTest)
{
...
#define BKSGE_ADD_POINTER_TEST(T1, T2)	\
	static_assert(std::is_same<bksge::add_pointer_t<T1>, T2>::value, "");	\
	static_assert(std::is_same<bksge::add_pointer<T1>::type, T2>::value, "")
