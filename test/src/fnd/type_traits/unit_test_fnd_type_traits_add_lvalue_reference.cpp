...
GTEST_TEST(TypeTraitsTest, AddLvalueReferenceTest)
{
...
#define BKSGE_ADD_LVALUE_REFERENCE_TEST(T1, T2)	\
	static_assert(std::is_same<bksge::add_lvalue_reference_t<T1>, T2>::value, "");	\
	static_assert(std::is_same<bksge::add_lvalue_reference<T1>::type, T2>::value, "")
