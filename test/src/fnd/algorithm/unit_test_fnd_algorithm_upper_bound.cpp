/**
 *	@file	unit_test_fnd_algorithm_upper_bound.cpp
 *
...
GTEST_TEST(AlgorithmTest, UpperBoundTest)
{
#if 1//!(defined(_MSC_VER) && (_MSC_VER >= 1911))
	{
		...
		BKSGE_CXX14_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 3);
		BKSGE_CXX14_CONSTEXPR_EXPECT_TRUE(ret == bksge::next(bksge::begin(a), 3));
	}
	{
		BKSGE_STATIC_CONSTEXPR int a[] { 1,1,2,2,2,3,3 };
		BKSGE_CXX14_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 2);
		BKSGE_CXX14_CONSTEXPR_EXPECT_TRUE(ret == bksge::next(bksge::begin(a), 5));
	}
	{
		BKSGE_STATIC_CONSTEXPR int a[] { 1,1,2,2,2,3,3 };
		BKSGE_CXX14_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 3);
		BKSGE_CXX14_CONSTEXPR_EXPECT_TRUE(ret == bksge::end(a));
	}
	{
		BKSGE_STATIC_CONSTEXPR int a[] { 1,3,5,6,7,9 };
		BKSGE_CXX14_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 7, pred1);
		BKSGE_CXX14_CONSTEXPR_EXPECT_TRUE(ret == bksge::next(bksge::begin(a), 5));
	}
#endif
	{
		BKSGE_STATIC_CONSTEXPR std::array<int, 5> a {{ 3,1,4,6,5 }};
		BKSGE_CXX17_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 3);
		BKSGE_CXX17_CONSTEXPR_EXPECT_TRUE(ret == bksge::next(bksge::begin(a), 2));
	}
	{
		BKSGE_STATIC_CONSTEXPR std::array<int, 3> a {{ 10,20,30 }};
		BKSGE_CXX17_CONSTEXPR_OR_CONST auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 10, pred2());
	...
	}
	{
		...
		auto ret = bksge::upper_bound(bksge::begin(a), bksge::end(a), 0, [](int x, int y) { return x < y; });
		EXPECT_TRUE(ret == bksge::next(bksge::begin(a), 0));
	}
}
