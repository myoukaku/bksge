...
{
	{
		...
		EXPECT_TRUE(o && *o == 42);
		EXPECT_TRUE(&*o != &i);
		auto o2 = bksge::make_optional<std::tuple<int, int>>(1, 2);
		static_assert(bksge::is_same<decltype(o2), bksge::optional<std::tuple<int, int>>>(), "");
		EXPECT_TRUE(o2 && std::get<0>(*o2) == 1 && std::get<1>(*o2) == 2);
