...
inline BKSGE_CXX14_CONSTEXPR bool LerpTest()
{
	using Vector3 = bksge::math::Vector3<T>;
	{
		...
		VERIFY(Vector3( 0.0,  0.0,  0.0) == Lerp(v1, v2, T(0.00)));
		VERIFY(Vector3( 2.5,  5.0,  7.5) == Lerp(v1, v2, T(0.25)));
		VERIFY(Vector3( 5.0, 10.0, 15.0) == Lerp(v1, v2, T(0.50)));
		VERIFY(Vector3( 7.5, 15.0, 22.5) == Lerp(v1, v2, T(0.75)));
		VERIFY(Vector3(10.0, 20.0, 30.0) == Lerp(v1, v2, T(1.00)));
	}
	{
		Vector3 v1 { -10,  10,  20 };
		Vector3 v2 {  10, -20, -30 };

		VERIFY(Vector3(-10.0,  10.0,  20.0) == Lerp(v1, v2, T(0.00)));
		VERIFY(Vector3( -5.0,   2.5,   7.5) == Lerp(v1, v2, T(0.25)));
		VERIFY(Vector3(  0.0,  -5.0,  -5.0) == Lerp(v1, v2, T(0.50)));
		VERIFY(Vector3(  5.0, -12.5, -17.5) == Lerp(v1, v2, T(0.75)));
		VERIFY(Vector3( 10.0, -20.0, -30.0) == Lerp(v1, v2, T(1.00)));
	}
	return true;
}
