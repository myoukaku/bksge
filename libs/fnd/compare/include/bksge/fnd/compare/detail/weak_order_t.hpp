...
private:
	...
	BKSGE_NOEXCEPT_DECLTYPE_RETURN(
		bksge::weak_ordering(weak_order(std::forward<T>(e), std::forward<U>(f))))
	...
	BKSGE_NOEXCEPT_DECLTYPE_RETURN(
		bksge::weak_ordering(bksge::compare_three_way()(std::forward<T>(e), std::forward<U>(f))))
#endif
	...
	BKSGE_NOEXCEPT_DECLTYPE_RETURN(
		bksge::weak_ordering(detail::strong_order_t{}(std::forward<T>(e), std::forward<U>(f))))

public:
	template <typename T, typename U>
	BKSGE_CONSTEXPR auto operator()(T&& e, U&& f) const
		BKSGE_NOEXCEPT_IF_EXPR(impl(std::forward<T>(e), std::forward<U>(f), bksge::detail::overload_priority<3>{}))
	->decltype((impl(std::forward<T>(e), std::forward<U>(f), bksge::detail::overload_priority<3>{})))
	{
		static_assert(bksge::is_same_as<bksge::decay_t<T>, bksge::decay_t<U>>::value, "");

		return impl(std::forward<T>(e), std::forward<U>(f), bksge::detail::overload_priority<3>{});
	}
};
