...
private:
	...
	>
	static BKSGE_CONSTEXPR bksge::partial_ordering
	...
	{
		return
			std::forward<T>(e) == std::forward<U>(f) ? bksge::partial_ordering::equivalent :
			std::forward<T>(e) <  std::forward<U>(f) ? bksge::partial_ordering::less       :
			std::forward<U>(f) <  std::forward<T>(e) ? bksge::partial_ordering::greater    :
