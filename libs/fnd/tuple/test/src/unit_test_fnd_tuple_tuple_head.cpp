...
GTEST_TEST(TupleTest, TupleHeadTest)
{
	...
	{
		BKSGE_CXX14_CONSTEXPR bksge::tuple<int, double, float> t1(1, 2.5, 3.5f);
		BKSGE_CXX14_CONSTEXPR_EXPECT_EQ(1, bksge::tuple_head(std::move(t1)));
	}
	{
	...
	}
	{
		...
		EXPECT_EQ(Noncopyable(1), bksge::tuple_head(std::move(t1)));
	}
	{
	...
	{
		bksge::tuple<bksge::string, float> t1("foo", 1.5f);
		EXPECT_EQ(bksge::string("foo"), bksge::tuple_head(std::move(t1)));
	}
	{
	...
	{
		BKSGE_CXX14_CONSTEXPR bksge::tuple<int> t1(42);
		BKSGE_CXX14_CONSTEXPR_EXPECT_EQ(42, bksge::tuple_head(std::move(t1)));
	}
	{
